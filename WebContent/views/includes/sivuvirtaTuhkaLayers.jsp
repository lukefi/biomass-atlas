<div>
	<h4 class="biomass_layer_title">Tuhkat 2015, t/v</h4>
	<div class="biomass_info_icon icon-info">
		<input type="hidden" value="">
	</div>
</div>
<table class="table select-column standard-width">
<thead>
	<tr>
		<td></td>
		<th></th>
	</tr>
</thead>
<tbody>
	<tr>
		<th scope="row">100101 Pohjatuhka, kuona ja kattilatuhka</th>
		<td><input type="checkbox" name="waste" value='1254'></td>
	</tr>
	<tr>
		<th scope="row">100102 Hiilen poltossa syntyvä lentotuhka</th>
		<td><input type="checkbox" name="waste" value='1255'></td>
	</tr>
	<tr>
		<th scope="row">100103 Turpeen ja käsittelemättömän puun polton lentotuhka</th>
		<td><input type="checkbox" name="waste" value='1256'></td>
	</tr>
	<tr>
		<th scope="row">100115 Rinnakkaispolton pohjatuhka, kuona ja kattilatuhka</th>
		<td><input type="checkbox" name="waste" value='1257'></td>
	</tr>
	<tr>
		<th scope="row">100117 Rinnakkaispolton lentotuhka</th>
		<td><input type="checkbox" name="waste" value='1258'></td>
	</tr>
</tbody>
</table>
